...
ggplot(storm_data, aes(x = precip_wk_in, gage_ht)) +
  geom_point()
...
# Sigma hat is 2.320378
```

**Q2: What are the estimates for** $\hat \beta_0, \hat \beta_1, \hat \sigma$ **?**

beta 0 hat is 3.9 \# example, this is the gage height in river with no rainfall, initial value

beta 1 hat is 2.2 \# example, this is the gage height in river after rainfall

sigma hat is 2.3 \# example, how much it deviated
...
y_bar <- mean(storm_data$gage_ht) # Mean of the gage height
...
y - Observed gage heights

y bar - Mean gage height (overall)

y hat - Expected gage height (given precipitation)
...
mod_summ <- summary(sim_mod) # mod_summ is a list
